...
def validate_task(task_id):
    ...
    db.session.commit()
    return jsonify({"message": message, "task": task.serialize(), "total_coins": child.total_coins}), 200
...
def redeem_reward(reward_id):
    ...
    db.session.commit()
    return jsonify({"message": "¡Premio canjeado!", "coins_remaining": child.total_coins}), 200
...
def create_tasks(child_id):
    data = get_json_payload()
    Task.query.filter_by(child_id=child_id).delete()
    for item in data:
...
def create_grand_prize(child_id):
    ...
    db.session.commit()
    return jsonify({"message": "¡Gran Premio actualizado!"}), 201
...
def add_coins(child_id):
    ...
    db.session.commit()
    return jsonify({"message": f"{coins} monedas añadidas", "total_coins": child.total_coins}), 200
...
def forgot_password():
    ...
    msg = Message('Prueba', html=f"<p>Recupera tu contraseña <a href={reset_url_password}>Aqui</a></p>", recipients=[data["email"]])
    mail.send(msg)
